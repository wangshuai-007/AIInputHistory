...
通知只在扩展确认本轮回复正常完成时触发。默认还会要求本轮总耗时达到 20 秒；低于最低耗时的快速回复会正常完成计时和历史记录，但不会自动发送通知。取消生成、请求异常、切换到其他已有对话、超时或页面被关闭时不会发送“完成”通知。

## 通知最低耗时

“通知最低耗时（秒）”默认值为 `20`，可设置为 `0–3600` 秒。自动完成通知只有在 `durationMs >= 最低耗时 × 1000` 时才会发送；例如默认设置下 `19.999s` 不通知，`20.000s` 会通知。设置为 `0` 表示不按耗时过滤。

这个阈值只影响自动完成通知，不影响请求耗时记录、历史记录或回复完成检测。设置页的“发送测试通知”会绕过最低耗时，以便独立验证浏览器权限、Webhook、Token 或自定义 HTTP 配置是否可用。
