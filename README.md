...
ChatGPT 正在回复时，再发送的新文本会进入 Queue，而不是打断当前回复。Queue 严格按 FIFO 自动处理，每条消息都支持编辑、撤回和“立即发送”。正常情况下上一轮回复确认结束后才发送下一条；点击“立即发送”可主动重试指定 Queue 项。自动发送只有在会话 DOM 中真正出现对应的新 user turn 后才算成功并出队，避免误点语音按钮或页面忽略 click 时丢消息。
