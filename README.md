...
- 悬浮按钮右键会打开操作菜单，可选择“隐藏悬浮按钮”或在请求异常卡住时“停止计时”；隐藏后可在扩展设置中重新开启。历史快捷键可录制为自定义组合键，也可以完全停用。
...
- 扩展弹窗可调整历史上限、发送记录上限和自动快照间隔，并可在中英文之间即时切换；另有独立的 AI 对话统计，按 AI 分别统计总对话、今日、本周、本月以及累计活跃天/周/月，并可单独清空。
- ChatGPT 支持 Queue 排队：正在回复时的新消息进入队列，可编辑、撤回或“立即发送”；上一轮确认结束后按 FIFO 自动发送。异常发送若长时间未被 ChatGPT 真正接收会释放队列锁并保留为可重试项，避免一条坏消息堵住后续队列。
- ChatGPT 回复完成通知默认关闭；可选择浏览器系统通知、Bark、Server酱、PushPlus、ntfy、Gotify、钉钉机器人、飞书机器人、企业微信机器人，或配置自定义 HTTP 请求。默认通知最低耗时为 20 秒，低于阈值的快速回复不会自动通知；测试通知不受该阈值限制。默认通知正文只取问题前 32 个字符，纯图片/附件请求使用专用提示；通知设置提供脱敏调试日志用于排查完成检测和渠道发送。
...
在扩展设置中开启“跟踪请求耗时”（默认关闭，仅 ChatGPT）。发送后悬浮按钮显示当前已用秒数，观察到新回复完成后恢复历史图标。历史记录右下角显示从发送到回复完成的总耗时和完成时刻；鼠标悬停或键盘聚焦记录时，发送时间精确到秒。悬浮计时异常卡住时可右键按钮选择“停止计时”，只结束扩展观察，不会停止 ChatGPT 正在生成的回复。
...
扩展设置页会独立记录 AI 对话统计，按 AI 类型分别展示总对话、今日、本周、本月以及累计活跃天/周/月，并提供“全部 AI”汇总。总对话按稳定会话地址去重；同一个会话在同一天多次发送只计一次当日使用，跨天继续使用会进入新一天统计但不会增加总对话。统计数据与输入历史分开保存在本地，“清空统计”不会删除历史，“清空历史”也不会清除统计。

## ChatGPT Queue 排队

ChatGPT 正在回复时，再发送的新文本会进入 Queue，而不是打断当前回复。Queue 严格按 FIFO 自动处理，每条消息都支持编辑、撤回和“立即发送”；撤回会把内容放回主输入框。Queue 的入队、撤回和程序化填充不会禁用上下键历史导航，用户重新输入或移动光标后，到达首尾视觉行仍可继续使用 ↑/↓ 浏览历史。正常情况下上一轮回复确认结束后才发送下一条；点击“立即发送”可主动重试指定 Queue 项。自动发送只有在会话 DOM 中真正出现对应的新 user turn 后才算成功并出队，避免误点语音按钮或页面忽略 click 时丢消息。

如果某条 Queue 自动发送长时间无法确认，扩展会把它标记为“发送未确认”，保留内容并释放队列锁，后面的正常消息仍可继续处理。Queue 数据仅保存在当前浏览器，并按 ChatGPT 会话 URL 隔离；新会话从临时 `/` 地址切换到 `/c/...` 后会迁移队列。完整行为见 [ChatGPT Queue 排队说明](docs/prompt-queue.md)。
